...
3. **Output Format**: Provide the task description and steps in two parts, formatted as JSON:
    ...
    - **Steps**: An array where each element represents a step, containing:
        - `step_description`: A concise description of the step which the action being performed in the format of verb phrases.
...
4. **Skill Library (Action Verbs)**
  ...
  Template 1 (Robot Movement - Navigation): For moving the robot base. Don't use this template for manipulating objects.
  Verbs: Go, Turn.

  Template 2 (Robot Movement - Vertical): For moving the robot base vertically without manipulating an object. Don't use this template for manipulating objects.
  Verbs: Lower, Lift.
